...
export default function LoginPage() {
  ...
  // Redirect as soon as auth store confirms a user — covers both
  // the "already logged in" case and the post-sign-in case.
  useEffect(() => {
    if (!loading && user) {
  ...
  const handleError = (err: AuthError) => {
    const messages: Record<string, string> = {
      ...
      "auth/invalid-api-key": "Firebase not configured — check .env.local.",
    };
    setError(messages[err.code] ?? `Sign-in failed (${err.code})`);
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2">Master Data Structures & Algorithms</p>
        </div>
        ...
        <div className="bg-gray-900 rounded-2xl p-8 shadow-2xl border border-gray-800">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              disabled={submitting}
              className="flex items-center justify-center gap-2 border border-gray-700 hover:bg-gray-800 disabled:opacity-60 text-white text-sm py-2.5 rounded-lg transition"
            >
              <GoogleIcon />
              Google
            </button>
